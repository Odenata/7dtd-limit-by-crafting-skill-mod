...
- **Mapping (from docs):** `Progression` has **`ProgressionClasses`** (Dictionary<string, ProgressionClass>). `ProgressionClass` has **`DisplayDataList`** (List<ProgressionClass+DisplayData>). `DisplayData` has **`QualityStarts`** (int[]; vanilla fills this from **`display_entry` `unlock_level`** at load — the shipped **`DisplayData`** type has **no** separate `unlock_level` field), **`Item`** / **`ItemName`**, **`UnlockDataList`** with per-child **`UnlockTier`** (**XML `unlock_tier` minus one**, 0-based column index); **`GetQualityLevel(System.Int32 level)`** returns quality at a given skill level. For rolled item quality Q, the mod uses **QualityStarts / optional test `unlock_level` field** first, then the minimum level L such that GetQualityLevel(L) >= Q (small integers must not be mistaken for Q when they are unlock column indices—e.g. **craftingSeeds**).
- **Mod:** `GameReflection.GetRequiredLevelForItem` uses the local player's Progression → ProgressionClasses[progressionLookupName] → DisplayDataList; finds DisplayData matching the item (by Item reference or ItemName); then reads required level via `GetRequiredLevelFromDisplayData` (see [`GATING_AND_RESTRICTIONS.md`](GATING_AND_RESTRICTIONS.md)). Returns 0 if no matching DisplayData or no quality.
...
- **Choke points:** Production blocking happens before committed moves: **`XUiC_ItemStack.HandleStackSwap`** / **`XUiM_PlayerInventory.AddItemToToolbelt`** for toolbelt and **`XUiM_PlayerEquipment.EquipItem`** / **`XUiC_EquipmentStack.HandleStackSwap`** for armor. Low-level `Inventory.SetItem` / `Equipment.SetSlotItem` are not safe blocking points because returning `false` there can lose items.

## Workstation / block open

- **Goal:** Prevent using the workstation UI when under level; show popup; **do not** use broad Harmony Prefix+skip on `BlockWorkstation.OnBlockActivated` (causes client UI lock until rejoin).
- **Confirmed:** The client reliably runs **`XUiC_WorkstationWindowGroup.OnOpen()`** when the crafting workstation UI opens; **`GameManager.workstationOpened`** is **not** always invoked on that path. **Mod:** **Postfix on `OnOpen`** — after vanilla OnOpen, if restricted: **`GUIWindowManager.CloseIfOpen(WorkstationWindow)`** (from **`BlockWorkstation.WorkstationData`** when known), **`XUi.GetWindowsByType` for `XUiC_WorkstationWindowGroup`**, close controller, then **`RestrictionFeedback.ShowRestrictionPopup`**. `BlockCollector.OnBlockActivated` is a narrow exception for Dew Collector / Apiary / Chicken Coop and uses `WorkstationOpenRestrictionPatch` prefixes.
...
| Food / Medical consume | **`ItemActionEat.ExecuteAction`**, **`ItemActionEat.ExecuteInstantAction`** | **ItemActionExecuteRestrictionPatch** when **Food** / **Medical** toggles are **true** in `Config.xml` (default **off** in shipped `Config.xml`). Hold-to-use is checked on **mouse release** (`ExecuteAction`); instant UI use uses **`ExecuteInstantAction`**. Progression: **`craftingFood`** / **`craftingMedical`**, composite **`display_entry`/`unlock_entry`**, not stack **Quality**; match **comma token**s in `UnlockData.ItemName` and resolve per-child **`UnlockTier`** (see `GATING_AND_RESTRICTIONS.md` §2.2.4). |
| Armor       | **`XUiM_PlayerEquipment.EquipItem(ItemStack)`** (block here); **`XUiC_EquipmentStack.HandleStackSwap()`** (block drag-drop) | **EquipItemRestrictionPatch** blocks at EquipItem. **EquipmentStackHandleStackSwapPatch** blocks at HandleStackSwap when the dragged stack (from XUi.dragAndDrop.CurrentStack) is restricted; item stays on cursor. We do not block at SetSlotItem to avoid item loss. |
