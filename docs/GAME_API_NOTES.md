...
| Food / Medical consume | **`ItemActionEat.ExecuteAction`**, **`ItemActionEat.ExecuteInstantAction`** | **ItemActionExecuteRestrictionPatch** when **Food** / **Medical** toggles are on in `Config.xml`. Hold-to-use is checked on **mouse release** (`ExecuteAction`); instant UI use uses **`ExecuteInstantAction`**. Progression: **`craftingFood`** / **`craftingMedical`** composite rows (per-item `unlock_level`, not stack quality). |
